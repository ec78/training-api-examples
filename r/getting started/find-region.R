...
# ── Configuration ──────────────────────────────────────────────────────────────
# These values come from Step 2 (get-datasets.R).
...
DATASET_ID <- 124            # The data year to use — replace with an ID from
                             # get-datasets.R (e.g., 124 = 2024 dataset)
...
get_token <- function(username, password) {
  ...
  # strip it here so we can re-add it cleanly in Authorization headers.
...
# The API returns a single region object (not an array) for the top level —
# this is the United States total. Wrap it in a list for uniform handling,
# and to confirm the correct dataset is loaded.
...
matches <- all_regions[sapply(all_regions, function(r) {
  grepl(REGION_SEARCH, r$description, ignore.case = TRUE)
})]
...
# ── Output ────────────────────────────────────────────────────────────────────
# The hashId from this output is what you will use in Step 5 (add-events.R)
# to assign your analysis to this geographic region.
if (length(matches) > 0) {
  cat(sprintf("\n  --> Use hashId '%s' in add-events.R as REGION_HASH_ID\n",
              matches[[1]]$hashId))
}
